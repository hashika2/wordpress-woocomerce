...
class ListTable extends WP_List_Table {
	...
	protected function get_bulk_actions() {
		...
		if ( array( 'trash' ) === $selected_status ) {
			...
			$actions = array(
				...
				'mark_pushed'           => __( 'Push the Orders', 'woocommerce' ),
			);
		}
	...
	public function render_wc_actions_column( WC_Order $order ): void {
		...
		if ( $order->has_status( array( 'pending', 'on-hold', 'processing', 'pushed' ) ) ) {
			$actions['pushed'] = array(
				'url'    => wp_nonce_url( admin_url( 'admin-ajax.php?action=woocommerce_mark_order_status&status=pushed&order_id=' . $order->get_id() ), 'woocommerce-mark-order-status' ),
				'name'   => __( 'Pushed', 'woocommerce' ),
				'action' => 'pushed',
			);
		}
